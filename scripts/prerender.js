...
const routes = [
  ...
  "/polityka-cookies",
  "/realizacje",
  ...REALIZACJE.map((r) => `/realizacje/${r.slug}`),
  "/katalog-wad",
  ...DEFECTS.map((d) => `/katalog-wad/${d.id}`),
  "/dobor-posadzki",
...
function injectHelmet(html, helmet) {
  ...
  if (helmet?.meta) {
    ...
    // Dołączono "og:image": bez tego strony z własnym, konkretnym zdjęciem
    // (realizacje, docelowo też katalog wad) i tak pokazywały ogólne
    // /og-image.jpg ze statycznego index.html, bo ta pętla go nie obsługiwała.
    ...
    // name="twitter:*" — Twitter/X Card. Ten sam problem co wyżej: bez tego
    // karty na X zawsze pokazywały ogólny tytuł/opis/zdjęcie strony głównej,
    // niezależnie od faktycznie udostępnianej podstrony.
    ["twitter:title", "twitter:description", "twitter:image"].forEach((name) => {
      const m = metaStr.match(new RegExp(`<meta[^>]*\\bname="${name}"[^>]*>`));
